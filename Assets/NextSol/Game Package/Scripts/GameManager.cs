...
public class GameManager : MonoBehaviour
{
    ...
    [HideInInspector]
    public int indexDecor = 0;
    public int[] listNumPartDecorCompleted;

    [HideInInspector]
    public GameObject effectSmokeFinish, effConfettiFinish;
        ...
        if (focus == true && !isShowAds)
        {
            ...
            if (DataManager.Instance.numTutorialStep > 1)
            {
                if (offlineTime >= 20)
                {
                    ...
                    moneyOffline = offlineTime * PopupAds.Instance.GetMaxIncome() / 100;
    ...
    public void SetUpShip(int index)
    {
        ...
        listShipWorking[index].GetComponent<ShipController>().numPartTransported = DataManager.Instance.listNumPartCompleted[index + 3 * LoadingManager.Instance.currentMap];
        listShipWorking[index].GetComponent<ShipController>().numPartCompleted = DataManager.Instance.listNumPartCompleted[index + 3 * LoadingManager.Instance.currentMap];
        ...
        if (isMainScene)
        {
            for (int i = 0; i < listShipWorking[index].transform.GetChild(2).transform.childCount; i++)
            {
                ...
                GameObject decor = Instantiate(listListDecorObject[index][i], listPosInstantiateDecorShipWork[index].position, Quaternion.identity, listPosInstantiateDecorShipWork[index]);
    ...
    public void MoveShip(int index)
    {
        StartCoroutine(CoWaitMoveShip(index));
    }
    ...
    public void SetUpWorkerForZone(int workZone)
    {
        ...
        if (listShipWorking[workZone].GetComponent<ShipController>().numPartTransported == listShipWorking[workZone].GetComponent<ShipController>().listPillarObject.Count)
        {
        ...
        {
            StartCoroutine(CoWaitSetUpWorker(workZone, Mathf.Min(10, (int)Mathf.Floor(DataManager.Instance.upgradeDatas.Upgrade_Worker[GameUIManager.Instance.listListUpgradeLevel[workZone][2]].worker))));
        }
    }
    ...
    public void UpgradeSpeedForZone(int zone)
    {
        ...
        else
        {
            ...
            {
                listListTweener[zone][i].DOTimeScale(speed + (speed * OperatorManager.Instance.listCaptains[zone].boostSpeed) + numSpeedBooster + numSpeedClick, 0.1f).SetEase(Ease.Linear);
            }
            for (int i = 0; i < listListWorker[zone].Count; i++)
            {
                listListWorker[zone][i].GetComponent<WorkerController>().anim.speed = speed + (speed * OperatorManager.Instance.listCaptains[zone].boostSpeed) + numSpeedBooster + numSpeedClick;
            }
        }
    }
